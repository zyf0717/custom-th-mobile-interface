...
describe('diagnosticsReport service', () => {
  it('builds report text with expected sections', () => {
    ...
    expect(report).toContain('teoheng-web-app issue report')
  ...
  it('builds deterministic report filename from provided date', () => {
    ...
    expect(fileName).toBe('teoheng-web-app-issue-report-2026-03-23T10-00-00-123Z.txt')
  })

  it('downloads text with blob URL lifecycle', () => {
    const createObjectURL = vi.fn().mockReturnValue('blob:teoheng-web-app')
    ...
    expect(revokeObjectURL).toHaveBeenCalledWith('blob:teoheng-web-app')
  })
})
